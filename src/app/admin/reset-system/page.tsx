...
const CONFIRM_PHRASE = 'اعادة تعيين النظام';
...
export default function AdminResetSystemPage() {
  ...
  const handleReset = async () => {
    if (confirmInput.trim() !== CONFIRM_PHRASE) {
      setError(`يجب كتابة "${CONFIRM_PHRASE}" للتأكيد`);
      return;
    }
    setLoading(true);
    setError('');
    try {
      const res = await fetch('/api/admin/reset-system', {
        ...
        body: JSON.stringify({ confirmPhrase: confirmInput.trim() }),
      });
        ...
        {step === 1 && (
          <>
            <p className="text-white/90 font-medium">
              تأكيد نهائي: سيتم الآن حذف جميع البيانات. لا يمكن التراجع.
            </p>
            <p className="text-white/70 text-sm">
              اكتب <span className="font-bold text-red-300">&quot;{CONFIRM_PHRASE}&quot;</span> للتأكيد:
            </p>
            <input
              type="text"
              value={confirmInput}
